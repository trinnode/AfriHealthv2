...
interface IAIPolicy {
    ...
    /// @notice Emitted when a policy is deleted
    event PolicyDeleted(
        bytes32 indexed policyId,
        address indexed deletedBy
    );

    /// @notice Emitted when active policy is set
    ...
    /// @notice Emitted when invoice is evaluated
    ...
    /// @notice Emitted when policy settings are initialized
    ...
    /// @notice Create a new AI policy for a patient
    ...
    function evaluateInvoice(
        ...
        address provider
    ) external returns (string memory result, string memory reason, uint256 confidence);
    ...
    /// @notice Get active policy ID for a patient
    /// @param patient Patient address
    ...
    /// @notice Get all policy IDs for a patient
    /// @param patient Patient address
    ...
    /// @notice Get evaluation history for a policy
    /// @param policyId Policy identifier
    ...
    /// @notice Initialize policy settings (admin only)
