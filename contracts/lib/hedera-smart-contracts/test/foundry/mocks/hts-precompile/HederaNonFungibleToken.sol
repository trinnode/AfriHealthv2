...
contract HederaNonFungibleToken is IHRC719, ERC721, Constants {
  ...
  HtsSystemContractMock internal constant HtsPrecompile =
    HtsSystemContractMock(HTS_PRECOMPILE);
  ...
  function mintRequestFromHtsPrecompile(
    ...
    external
    onlyHtsPrecompile
    returns (int64 newTotalSupply, int64 serialNumber)
  {
    (, IHederaTokenService.TokenInfo memory nftTokenInfo) = HtsPrecompile
      .getTokenInfo(address(this));
  ...
  function setApprovalForAll(address operator, bool approved) public override {
    address sender = msg.sender;
    int64 responseCode = HtsPrecompile.preSetApprovalForAll(
      sender,
      operator,
      approved
    );
