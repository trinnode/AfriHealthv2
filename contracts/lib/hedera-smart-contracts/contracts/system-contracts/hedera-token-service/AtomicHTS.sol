...
contract AtomicHTS is HederaTokenService {
  ...
  event BatchAssociateGrantKYCTransfer(
    int associateResponseCode,
    int grantKYCResponseCode,
    int transferTokenResponseCode
  );
  event BatchApproveAssociateGrantKYCTransferFrom(
    int transferTokenResponseCode,
    int approveResponseCode,
    int associateResponseCode,
    int grantKYCResponseCode,
    int transferFromResponseCode
  );
  event BatchUnfreezeGrantKYCTransferFreeze(
    int unfreezeTokenResponseCode,
    int grantKYCResponseCode,
    int transferTokenResponseCode,
    int freezeTokenResponseCode
  );
  event BatchWipeMintTransfer(
    int wipeTokenResponseCode,
    int mintTokenResponseCode,
    int transferTokenResponseCode
  );
   ...
   */
  function batchAssociateGrantKYCTransfer(
    address token,
    address sender,
    address receiver,
    int64 amount
  ) external {
    int associateResponseCode = HederaTokenService.associateToken(
      receiver,
      token
    );
    require(
    ...
    /// an account needs to be granted the KYC of the HTS token for it to receive the token
    int grantKYCResponseCode = HederaTokenService.grantTokenKyc(
      token,
      receiver
    );
    require(
      grantKYCResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to grant token KYC.'
    );

    int transferTokenResponseCode = HederaTokenService.transferToken(
      token,
      sender,
      receiver,
      amount
    ...
    require(
      transferTokenResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to transfer token.'
    );

    emit BatchAssociateGrantKYCTransfer(
      associateResponseCode,
   ...
   *
   * because .approve() can only grant allowances to spender on behalf of the caller, and in this case
  ...
  function batchApproveAssociateGrantKYCTransferFrom(
    ...
    /// it is necessary for the spender to be associated and granted token KYC to receive fund.
    address spender = address(this);
    int associateContractResponseCode = HederaTokenService.associateToken(
      spender,
      token
    );
    require(
      associateContractResponseCode == HederaResponseCodes.SUCCESS ||
        associateContractResponseCode ==
        HederaResponseCodes.TOKEN_ALREADY_ASSOCIATED_TO_ACCOUNT,
      'Failed to associate token.'
    );

    int grantKYCContractResponseCode = HederaTokenService.grantTokenKyc(
      token,
      spender
    );
    require(
      grantKYCContractResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to grant token KYC.'
    );
    ...
    int approveResponseCode = HederaTokenService.approve(
      token,
      spender,
      allowance
    );
    require(
      ...
      'Failed to grant token allowance.'
    );
    ...
    int transferFromResponseCode = this.transferFrom(
      token,
      spender,
      receipient,
      allowance
    ...
    require(
      transferFromResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to transfer token.'
    );
   ...
   */
  function batchUnfreezeGrantKYCTransferFreeze(
    address token,
    address sender,
    address receiver,
    int64 amount
  ) external {
    int unfreezeTokenResponseCode = HederaTokenService.unfreezeToken(
      token,
      receiver
    );
    require(
      unfreezeTokenResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to unfreeze token.'
    );
    ...
    int freezeTokenResponseCode = HederaTokenService.freezeToken(
      token,
      receiver
    );
    require(
      freezeTokenResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to freeze token.'
    );

    emit BatchUnfreezeGrantKYCTransferFreeze(
      unfreezeTokenResponseCode,
      grantKYCResponseCode,
      transferTokenResponseCode,
      freezeTokenResponseCode
    );
  }
  ...
  function batchWipeMintTransfer(
    ...
    int wipeTokenResponseCode = HederaTokenService.wipeTokenAccount(
      token,
      owner,
      wipedAmount
    );
    require(
      wipeTokenResponseCode == HederaResponseCodes.SUCCESS,
      'Failed to wipe token.'
    );

    (int mintTokenResponseCode, , ) = HederaTokenService.mintToken(
      token,
      mintAmount,
      metadata
    );
    require(
      mintTokenResponseCode == HederaResponseCodes.SUCCESS,
    ...
    emit BatchWipeMintTransfer(
      wipeTokenResponseCode,
      mintTokenResponseCode,
      transferTokenResponseCode
    );
  }
   ...
   */
  function batchAssociateMintGrantTransfer(
    address token,
    address sender,
    address receiver,
    int64 amount
