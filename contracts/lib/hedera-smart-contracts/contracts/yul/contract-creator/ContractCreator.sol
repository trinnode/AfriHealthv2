...
/**
 * The unit tests for the `ContractCreator` contract will utilize a predefined bytecode of a contract that inherits the `ITargetContract` interface.
 */
...
contract ContractCreator {
  ...
  function createNewContract(bytes memory bytecode) external payable {
    ...
    assembly {
      ...
      // get actual bytecode
      //: This is done as `add(bytecode, 0x20)` because the first 32 bytes of the `bytecode` are often used to store the length of the bytecode,
