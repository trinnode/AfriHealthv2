...
contract ContractCaller {
  ...
  function call(
    uint256 gasLimit,
    address payable _targetContractAddress,
    bytes memory input
  ) external payable {
    bool success;
  ...
  function callCode(
    ...
    bool success;
    assembly {
      /// callcode uses the code from `_targetContractAddress` to update current contract's states
      success := callcode(
        gasLimit,
        _targetContractAddress,
        callvalue(),
  ...
  function delegateCall(
    ...
    bool success;
    assembly {
      /// delegatecall uses the code from `_targetContractAddress` to update current contract's states
