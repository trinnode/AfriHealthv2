...
contract VoteProxy is ERC1967Proxy, Context {
  ...
  ) ERC1967Proxy(implementationContract, '') {
   ...
   *
   * The IMPLEMENTATION_SLOT is obtained as bytes32(uint256(keccak256('eip1967.proxy.implementation')) - 1)
   */
   ...
   *
   * The internal _implementation() utilizes ERC1967Utils.getImplementation();
   */
  ...
  function upgradeToAndCall(
    ...
    bytes memory data
  ) external onlyProxyAdmin(_msgSender()) {
   ...
   *
   * The ADMIN_SLOT is obtained as bytes32(uint256(keccak256('eip1967.proxy.admin')) - 1)
   */
